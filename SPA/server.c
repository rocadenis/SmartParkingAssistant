...
void freeParkingSpot(int spotNumber, Client *client) {
    ...
    json_t *root = json_loads(data, 0, &error);
    if (!root) {
        fprintf(stderr, "Eroare la parsarea fișierului JSON pentru eliberarea locului: %s\n", error.text);
        pthread_mutex_unlock(&lock);
        exit(EXIT_FAILURE);
    }
...
//logout
        ...
        send(client_sock, buffer, strlen(buffer), 0);
//init
    } else if (strncmp(command, "Initializare sistem", 4) == 0) {
...
restart:
    ...
    if(isCreateAccount == 1){
            ...
            if ( result == 0 ) {
                ...
                strncpy(buffer, "Autentificare reușită!",25);
